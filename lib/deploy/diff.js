...
function getFilesToUpload(localDirectory, remotes) {
  ...
  return Promise.all(promises).then(files => {
    return files.filter(file => {
      ...
      if (!match) {
        console.debug(`Local file added: ${ file.path }`);
        return true;
      } else if (file.hash !== match.document.properties.md5) {
        console.debug(`Local file changed: ${ file.path }`);
        return true;
      }
...
function getFoldersToUpload(localDirectory, remotes) {
  ...
  return folders.filter(folder => {
    ...
    if (!exists) {
      console.debug(`Local folder added: ${ folder.path }`);
      return true;
    }
